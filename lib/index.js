		...
		if (!isIgnoredMethod) {
			...
			if (isConditionalRequest) {
				...
				if (rejectedTags !== undefined) {
					if (matches(weakTag, rejectedTags)) throw method === 'GET' || method === 'HEAD' ? response304(etagHeader, lastModifiedHeader) : 412;
...
const response304 = (etagHeader, lastModifiedHeader) => {
	const headers = {};
	if (etagHeader !== undefined) headers['etag'] = etagHeader;
	if (lastModifiedHeader !== undefined) headers['last-modified'] = lastModifiedHeader;
	return [304, headers];
};
